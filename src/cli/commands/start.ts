...
export const startCommand = new Command()
  ...
  .action(async (options) => {
    const spinner = new Spinner({ message: 'Starting Claude-Flow...' });
    spinner.start();
...
function setupEventListeners(): void {
  ...
  eventBus.on(SystemEvents.AGENT_SPAWNED, ({ agentId, profile }) => {
    console.log(colors.blue('→'), `Agent spawned: ${colors.cyan(agentId)} (${profile.type})`);
  });

  eventBus.on(SystemEvents.AGENT_TERMINATED, ({ agentId, reason }) => {
  ...
  eventBus.on(SystemEvents.TASK_COMPLETED, ({ taskId }) => {
    console.log(colors.green('✓'), `Task completed: ${colors.yellow(taskId)}`);
  });

  eventBus.on(SystemEvents.TASK_FAILED, ({ taskId, error }) => {
    console.log(colors.red('✗'), `Task failed: ${colors.yellow(taskId)} - ${error.message}`);
  });

  eventBus.on(SystemEvents.SYSTEM_ERROR, ({ component, error }) => {
